...
# LeakSanitizer off — link the sanctioned build-time hook (mayhem/lsan_off.cc, __lsan_is_turned_off()
# returning 1), NOT a compiled-in sanitizer-default-options override (FORBIDDEN — Mayhem alone owns
# ASAN/LSan runtime options) and NOT ASAN_OPTIONS in the Mayhemfile. embed's main()
# (src/tools/embed-standalone.c) reads the whole input into a malloc'd buffer and never frees it before
# returning, so LSan reports a "leak" on EVERY input — the default seed and every valid program
# included. That floods the fuzzer with leaks before it can reach a real memory defect. We disable ONLY
# leak detection; ASan's real memory-safety checks (overflow, use-after-free, …) AND halting UBSan stay
# fully ON, so genuine defects in the read/encode path still crash. Only emitted when ASan is active
# (skipped for the empty-sanitizer build).
ASAN_OPTS_OBJ=""
if printf '%s' "$SANITIZER_FLAGS" | grep -q address; then
  $CC -x c++ -std=c++11 $SANITIZER_FLAGS -c "$SRC/mayhem/lsan_off.cc" -o /tmp/lsan_off.o
